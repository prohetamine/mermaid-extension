...
const observer = new MutationObserver(async mutationRecords => {
  ...
  try {
    isToken = !!node.querySelector('div[dm-init-bg="#ff3"]')

    if (isToken) {
      const spanNode = [...node.querySelectorAll('span')].find(span => {
        try {
          return !!span.innerText.match(/tipped \d+ token/)
        } catch (err) {
          return false
        }
      })
      ...
      if (!!spanNode) {
        tokenCount = parseInt(spanNode.innerText.match(/\d+/))
      }
    }
  } catch (err) {}
  ...
  chrome.storage.local.set({
    chaturbateEvent: ({
      ...
      parseMessageEvent: {
        isModel,
        modelUsername: isModel ? username : '',
        isNotice,
        isUser,
